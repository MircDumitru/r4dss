...
In this chapter, you'll learn tools for iteration, repeatedly performing the same action on different objects. Iteration in R generally tends to look rather different from other programming languages because so much of it is implicit and we get it for free.

For example, if you want to double a numeric vector `x` in R, you can just write `2 * x`. In most other languages, you'd need to explicitly double each element of `x` using sort of `for` loop.
...
## Modifying multiple columns {#sec-modifying-multiple-columns}
...
The first argument to `across()`, `.cols`, selects the columns to transform. This uses the same specifications as `select()`, @sec-columns, so you can use functions like `starts_with()` and `ends_with()` to select columns based on their name.
...
The second argument to `across()` defines how each column will be transformed. In simple cases, as above, this will be a single existing function. This is a pretty special feature of R: we're passing one function (`median`, `median`, `str_flatten`,...) to another function (`across`). This is one of the features that makes R a functional programming language.

It's important to note that we're passing this function to `across()`, so `accross()` can call it; we're not calling it ourselves. That means the function name should never be followed by `()`. If you forget, you will get an error:
...
In more complex cases, you might want to supply additional arguments or perform multiple transformations. Let's motivate this problem with a simple example: what happens if we have some missing values in our data? `median()` propagates those missing values, giving us a sub-optimal output:
...
This is a little verbose, so R comes with a hand shortcut: for this sort of throw away, or **anonymous**[^iteration-1], function you can replace `function` with `\`[^iteration-2]:

[^iteration-1]: Anonymous, because we never explicitly gave it a name with `<-`. Another term programmers use for this is "lambda function".

[^iteration-2]: In older code you might see syntax that looks like `~ .x + 1`. This is another way to write anonymous functions but it only works inside tidyverse functions and always uses the variable name `.x`. We now recommend the base syntax, `\(x) x + 1`.
...
When we remove the missing values from the `median()`, it would be nice to know just how many values we removed. We can find that out by supplying two functions to `across()`: one to compute the median and the other to count the missing values. You supply multiple functions by using a named list to `.fns`:
...
If you look carefully, you might intuit that the columns are named using a glue specification (@sec-creating-many-strings) like `{.col}_{.fn}` where `.col` is the name of the original column and `.fn` is the name of the function. That's not a coincidence. As you'll learn in the next section, you can use the `.names` argument to supply your own glue spec.

### Column names

The result of `across()` is named according to the specification provided in the `.names` argument. We could specify our own if we wanted the name of the function to come first[^iteration-3].
...
The `.names` argument particularly important when you use `across()` with `mutate()`. By default, the output of `across()` is given the same names as the inputs. This means that `across()` inside of `mutate()` will replace existing columns. For example, here we use `coalesce()` to replace `NA`s with `0`:
...
```{r}
# same as df_miss |> filter(is.na(a) & is.na(b) & is.na(c) v is.na(d))
df_miss |>
  filter(if_all(a:d, is.na))
```
...
`across()` also makes it easy to supply multiple columns in a single argument because the first argument uses tidy-select; you just need to remember to embrace that argument, as we discussed in @sec-data-frame-functions. For example, this function will compute the means of numeric columns by default. But by supplying the second argument, you can choose to summarize just selected columns:
...
And if we wanted the same structure as `across()` you could pivot again:
...
df_long |>
  group_by(group) |>
  summarise(mean = weighted.mean(vals, whgts))
```
...
1.  Practice your `across()` skills by:
    ...
    1.  Computing the number of unique values in each column of `palmerpengions::penguins`.
...
2.  What happens if you use a list of functions in across() but you don't name them? How is the output named?

    Using `df_miss` to compute the median and the number of missing values across all columns, first time with named functions and second time with unnamed functions:

    ```{r}
    # Named functions inside the list
    df_miss |>
      summarise(
        across(
          everything(),
    ...
    ```{r}
    # Unnamed functions inside the list
    df_miss |>
      summarise(
        across(
          everything(),
    ...
    Using a list of function in `across()` without naming them will still work, and the names assigned to the function are simply the numbers corresponding to the the functions (i.e. `1` for the first function, `2` for the second function and so on). The convention for unnamed functions is `{.col}_x`, with `x` representing the function number in the order it appears in the list.

3.  Adjust `expand_dates()` to automatically remove the date columns after they've been expanded. Do you need to embrace any argument?

    ```{r}
    # Dates data frame
    df_date <- tibble(
      name = c("Alice", "Bob", "James"),
      date1 = ymd(c("1993-05-21", "1994-10-30", "1995-11-12")),
      date2 = ymd(c("2000-03-11", "2001-04-19", "2002-04-11"))
    )
    df_date  
    ```

    One way to write the function is a one parameter function (the data frame), and select the column across which the computations are done (i.e. the date type columns) inside the function. The dates columns can be removed simply by using `.keep = "none"` in `mutate()`. In this case, there is no need to embrace any argument.

    ```{r}
    # Function for dates expanding
    expand_dates2 <- function(df) {
      df |>
        mutate(
          across(
            where(is.Date),
    ...
    Another way to write the function is a two parameter function: the data frame and the variables over which we do the computation, meaning that the selection of the columns is done outside the function. In this case, there is necessary to embrace the second function argument, represeting the dataframe columns over which the expansion is done. Like in the first case, the columns are removed using `.keep = "none".`

    ```{r}
    # Function for dates expanding
    expand_dates3 <- function(df, vars = where(is.Date)) {
      df |>
        mutate(
          across(
            {{ vars }},
...
4.  Explain what each step of the pipeline in this function does. What special feature of `where()` are we taking advantage of?
    ...
    show_missing <- function(df, group_vars, summary_vars = everything()) {
      df |>
        # data frame is grouped by group_vars variable.
        # since group_by is using data-masking pick() is used to 
        # force tidy-selection
        group_by(pick({{ group_vars }})) |>
        summarise(
          # across is used over summary_vars 
          # lambda function is defined since we want the sum of missing values
          across({{ summary_vars }}, \(x) sum(is.na(x))),
          .groups = "drop"
        ) |>
        # selects all the columns for which at least one entry is positive
        # the feature of where() function that is used as a selection helper
        # selecting variables for which a function returns TRUE
...
In the previous section, you learned how to use `dplyr::across()` to repeat a transformation on multiple columns. In this section you will learn how to use `purrr::map()` to do something to every file in a directory. Let's start with a little motivation: imagine you have a directory full of excel spreadsheets you want to read. You could do it with copy and paste:

```{r}
#| eval: false
data2019 <- readxl::read_excel("book-data/y2019.xlsx")
data2020 <- readxl::read_excel("book-data/y2020.xlsx")
data2021 <- readxl::read_excel("book-data/y2021.xlsx")
data2022 <- readxl::read_excel("book-data/y2022.xlsx")
```
...
You can imagine that this would get tedious quickly, especially if you had hundreds of files, not just four. The following sections show you how to automate this sort of task. There are three basic steps:
...
We'll discuss how you can handle situations of increasing heterogeneity, where you can't do exactly the same thing to every file.
...
-   `pattern` is a regular expression used to filter the file names. The most common pattern is something like `[.]xlsx$` or `[.]csc$` to find all files with a specified extension.
...
gapminder_1952 <- readxl::read_excel("book-data/gapminder/1952.xlsx")
gapminder_1957 <- readxl::read_excel("book-data/gapminder/1957.xlsx")
gapminder_1962 <- readxl::read_excel("book-data/gapminder/1962.xlsx")
...,
gapminder_2007 <- readxl::read_excel("book-data/gapminder/2007.xlsx")
```
...
files <- list(
  readxl::read_excel("book-data/gapminder/1952.xlsx"),
  readxl::read_excel("book-data/gapminder/1957.xlsx"),
  readxl::read_excel("book-data/gapminder/1962.xlsx"),
  ...,
  readxl::read_excel("book-data/gapminder/2007.xlsx")
)
```
...
Now that you have these data frames in a list, how to do you get one out? You can use `files[[1]]` to extract the i^th^ element:

```{r}
files[[3]]
```

We'll come back to `[[` in more detail in @sec-selecting-a-single-element.

### `purrr:map()` and `list_rbind()`

The code to collect those data frames in a list "by hand" is basically just as tedious to type as code that reads the files one-by-one. Happily we can use `purrr:map()` to make even better use of our `paths` vector. `map()` is similar to `across()`, but instead of doing something to each column in a data frame, it does something to each element of a vector.
...
What if want to pass in extra arguments to `read_excel()`? We use the same technique that we used with `across()`. For example, it's often useful to peek at the first few rows of the data with `n_max = 1`:
...
1.  First, we name the vector of paths. The easiest way to do this is with the `set_names()` function, which can take a function. Here we use `basename()` to extract just the file name from the full path:
    ...
    Those names are automatically carried along by all the maps functions, so the list of data frames will have those same names:
    ...
    #| eval: false
    files <- list(
      "1952.xlsx" = readxl::read_excel("book-data/gapminder/1952.xlsx"),
      "1957.xlsx" = readxl::read_excel("book-data/gapminder/1957.xlsx"),
      "1962.xlsx" = readxl::read_excel("book-data/gapminder/1962.xlsx"),
      ...,
      "2007.xlsx" = readxl::read_excel("book-data/gapminder/2007.xlsx"),
    )
    ```
...
2.  Secondly, we use the `names_to` argument to `list_rbind()` to tell it to save the names into a new column called `year` then use `readr::parse_number()` to extract the number from the string.
...
Now that you've done all this hard work to get a nice tidy data frame, it's a great time to save your work:
...
Now when you come back to this problem in the future, you can read in a single csv file. For large and richer datasets, using parquet might be a better choice than `csv` as discussed in @sec-the-parquet-format.
...
If your input data files change over time, you might consider learning a tool like [argets](https://docs.ropensci.org/targets/) to set up your data cleaning code to automatically re-run whenever one of the input files is modified.

### Many simple iterations

Here we've just loaded the data directly from disk, and were lucky enough to get a tidy dataset. In most cases, you'll need to do some additional tidying, and you have two basic options: you can do one round of iteration with a complex function or to do multiple rounds of iteration with simple functions. in our experience most folks reach first for one complex iteration but you're often better by doing multiple simple iterations.
...
We recommend this approach because it stops you getting fixated on getting the first file right before on the rest. By considering all of the data when doing tidying and cleaning, you're more like to think holistically and end up with a higher quality result.
...
If the files have heterogeneous formats, you might need to do more processing before you can successfully merge them. Unfortunately, we're now going to leave you to give that out on your own, but you might what to read about `map_if()` and `map_at()`:
...
Now you have all the data that can be read easily, and it's time to tackle the hard part of figuring out why some files failed to load and what to do about it. Start by getting the path that failed:
...
## Saving multiple outputs {#sec-saving-multiple-outputs}
...
This would work well here, but we don't have csv files, instead we have except spreadsheets. So we're going to have to do it "by hand". Learning to do it by hand will also help you when you have a bunch of CSVs and the database that you're working with doesn't have one function that will load them all in.

We need to start by creating a table that we fill in with data. The easiest way to do this is by creating a template, a dummy data frame that contains all the columns we want, but only a sampling of the data.

For the gamminder data, we can make that template by reading a single file and adding the year to it:
...
Now we can connect to the database, and use `DBI::CreateTable()` to turn our template into a database table:
...
This is a little difference to our previous uses of `map()` because there are two arguments that are changing, not just one. That means we need a function: `map2()`, which varies both the first and second arguments. And because we again don't care about the output, we want `walk2()` rather than `map2()`. That gives us:
...
by_clarity <- by_clarity |>
  mutate(
    ...
    path = str_glue("figs/clarity-{clarity}.png")
  )
```
...
In this chapter, you've seen how to use explicit iteration to solve three problems that come up frequently when doing data science: manipulating multiple columns, reading multiple files, and saving multiple outputs. But in general iteration is a super power: if you know the right iteration technique, you can easily go from fixing one problem, to fixing all the problems. Once you've mastered the techniques in this chapter we highly recommend learning more by reading the [Functionals](https://adv-r.hadley.nz/functionals.html) chapter of [Advanced R](https://adv-r.hadley.nz) and consulting the [purr](https://purrr.tidyverse.org) website.

If you know much about iteration in other languages, you might be surprised that we didn't discuss the `for` loop. That is because R's orientation towards data analytics changes how we iterate: in some cases you can rely on an existing idiom to do something to each columns or each group. And when you can't, you an often use a functional programming tool like `map()` that does something to each element of a list.
