...
This is a good place to remind you that the tidyverse is not the only way to solve data science problems. We teach the tidyverse in this book because **tidyverse** packages share a common design philosophy, increasing the consistency across functions, and making each new function or package a little easier to learn and use. It's not possible to use the tidyverse without using base R, so we've actually already taught you a lot of base R functions: from `library()` to load packages to `sum()` and `mean()` for numeric summaries, to the factor, data, and POSIXct data types, and of course all the basic operators like `+`, `-`, `/`, `*`, `|`, `&`, and `!`. What we haven't focused on so far is base R workflows, so we will highlight a few of those in this chapter.

After you read this book, you'll learn other approaches to the same problems using base R, `data.table`, and other packages. You will undoubtedly encounter these other approaches when you start reading R code written by others, particularly if you're using StackOverflow. It's 100% okay to write code that uses a mix of approaches and don't let anyone tell you otherwise.

In this chapter we'll focus on four big topics: subsetting with `[`, subsetting with `[[` and `$`, the apply family of functions and `for` loops. To finish off we will briefly discuss two essential plotting functions.

### Prerequisites

This package focuses on base R so doesn't have any real prerequisites, but we'll load the the tidyverse in order to explain some of the differences.
...
4.  **A character vector**. If you have a named vector, you can subset it with a character vector:
    ...
    As wit subsetting with positive positive integers, you can use a character vector to duplicate individual entries:
...
There are quite a few different ways[^a-field-guide-to-base-r-1] that you can use `[` with a data frame, but the most important way is to select rows and columns independently with `df[rows, cols]`. Here `[rows]` and `[cols]` are vectors as described above. For example `df[rows, ]` and `df[, cols]` select just rows or just columns, using the empty subset to preserve the other dimension.

[^a-field-guide-to-base-r-1]: Read <https://adv-r.hadley.nz/subsetting.html#subset-multiple> to see how you can also subset a data frame like it is a 1d object and how you can subset it with a matrix.
...
One way to avoid this ambiguity with `data.frame` is to explicitly specify `drop = FALSE`:
...
-   `filter()` is equivalent to subsetting the rows with a logical vector, taking care to exclude missing values:
    ...
    df <- tibble(
      x = c(2, 1, 1, 1, NA),
      y = letters[sample(5, replace = TRUE)],
      z = runif(5)
    )
...
Base R also provides a function that combines the features of `filter()` and `select()` called `subset()`[^a-field-guide-to-base-r-2]:

[^a-field-guide-to-base-r-2]: But it doesn’t handle grouped data frames differently and it doesn’t support selection helper functions like `starts_with()`.
...
1.  Create functions that take a vector as input and return:
    ...
    a.  The elements at even-numbered positions.
        ...
        ```{r}
        # Testing vectors 
        # test for NA containing vectors
        x <- sample(c(1:5, NA), 7, replace = TRUE)
        # test for one length vectors
        y <- c(1) 
        # test for zero length vectors
        z <- vector(mode = "logical", length = 0) 
        ```

        ```{r}
        # Function
        even_numbered_positions <- function(x) {
          if(length(x) < 2) {
        ...
        list(
        "x" = x,
        "f(x)" = even_numbered_positions(x)
        )
            
        list(
        "x" = y,
        "f(x)" = even_numbered_positions(y)
        )
            
        list(
        "x" = z,
        "f(x)" = even_numbered_positions(z)
        )
        ```
    ...
    b.  Every element except the last value.
        ...
        list(
        "x" = x,
        "f(x)" = except_last_value(x)
        )
            
        list(
        "x" = y,
        "f(x)" = except_last_value(y)
        )
            
        list(
        "x" = z,
        "f(x)" = except_last_value(z)
        )
        ```
    ...
    c.  Only even values (and no missing values).
        ...
        ```{r}
        # Function
        only_even_no_NA <- function(x) {
          if(typeof(x) %in% c("numeric", "integer", "double")) {
            x[which(x %% 2 == 0)]
          }
          else {
            NULL
        ...
        list(
        "x" = x,
        "f(x)" = only_even_no_NA(x)
        )
             
        list(
        "x" = y,
        "f(x)" = only_even_no_NA(y)
        )
            
        list(
        "x" = z,
        "f(x)" = only_even_no_NA(z)
        )
        ```

2.  Why is `x[-which(x > 0)]` not the same as `x[x <= 0]`. Read the documentation for `which()` and do some experiments to figure it out.

    The two are not the same because `which(cond)` not only selects the positions for which `cond` is true but it also removes the `NA` values.

    In general, `x[which(cond)]` is different from `x[cond]` for vectors containing `NA` values, the `NA` values being removed when using `which` and being kept when using just the condtion. The next chunk shows that `x[which(x > 0)]` is in general different from `x[x > 0]`:
    ...
    For, `x[-which(cond)]` the difference from `x[!cond]` comes from extreme cases, like only `NA` values vectors. It also comes from handling `NaN` differently: kept as `NaN` when using which, transformed to `NA` when not:
...
## Selecting a single element with `$` and `[[` {#sec-selecting-a-single-element}

`[`, which selects many elements is paired wit `[[` and `$`, which extract a single element. In this section, we'll show you how to use `[[` and `$` to pull columns out of data frames, discuss a couple more differences between `data.frame` and `tibble` and emphasize some important differences between `[` and `[[` when used with lists.
...
Using the base R function `with()`, to avoid repeating diamonds:
...
Using the base R function `transform()` (the inspiration for `dplyr`'s `mutate()`):
...
diamonds <- diamonds |>
  transform(ppc = price/carat)
```

Using the base R function `within()`, similar to `transform()`, but uses assignment rather than argument matching (can also use `=` here, since `=` is equivalent to `<-` outside of a function call)

```{r}
#| eval: false

diamonds <- within(diamonds, {ppc <- price/carat})

# Equivalently, using the pipe operator `|>`:
diamonds <- diamonds |>
  within({ppc <- price/carat})
```
...
`dplyr` also provides an equivalent to `[[`/`$` that we didn't mention in @sec-data-transformation: `pull()`. `pull()` takes either a variable name or a variable position and returns just that column. That means we could rewrite the above code to use the pipe:
...
There are a couple of important differences between `tibble`s and base `data.frame`s when it comes to `$`. Data frames match the prefix of any variable name (so-called **partial matching**) and don't complain if a column doesn't exist:
...
-   `[` extracts a sub-list. It doesn't matter how many elements your extract, the result will always be a list:
...
The difference between `[` and `[[` is particularly important for lists because `[[` drills down into the list while `[` returns a new, smaller list. If `x` is a list, `x[i]` is a list containing the i^th^ element in `x` and `x[i:j]` is a list containing the elements from `i` to `j`. x\[\[i\]\] extracts the element `i`.

The same principle applies when you use 1d `[` with a data frame: `df["x"]` returns a one-column data frame and `df[["x"]]` returns a vector.

### Exercises

1.  What happens when you use `[[` with a positive integer that's bigger than than length of the vector? What happens when you susbset with a name that doesn't exist?
    ...
    When using `[[` with a positive integer larger than the length of a vector or a list, an error is thrown: "subscript out of bounds":
    ...
    When using `[[` with a positive integer larger than the number of columns of a tibble, an error is thrown "Can't extract columns past the end".
    ...
    Subsetting a vector with a name that doesn't exist returns an error, "subscript out of bounds".
    ...
    Subsetting a list, dataframe (`data.frame`) or tibble with a name that doesn't exist returns `NULL`:
...
2.  What would `pepper[[1]][1]` be? What about `pepper[[1]][[1]]`?
    ...
    `pepper[[1]]` extracts the first element in `pepper`, i.e. the first pepper packet. `pepper[[1]][1]` selects the first pepper grain together with the package (i.e. list). `pepper[[1]][[1]]` selects the first pepper grain.
...
In @sec-iteration, you learned tidyverse techniques for iteration like `dplyr::across()` and the map family of functions. In this section, you'll learn about the base equivalents, the **apply family**. In this context apply and map are synonyms because another way of saying "map a function over each element of a vector" is "apply a function over each element of a vector". Here we'll give you a quick overview of this family so you can recognize them in the wild.

The most important of this family is `lapply()`, which is very similar to `purrr:map()`. In fact, because we haven't used any of `map()`'s more advanced features, you can replace every `map()` call in @sec-iteration with `lapply()`.
...
# Transform each column with lapply() then replace the original values
df[, num_cols] <- lapply(df[, num_cols, drop = FALSE], \(x) x * 2)
df
```

The code above uses a new function, `sapply()`. It's similar to `lapply()` but it always tries to simplify the result, hence the `s` in its name, here producing a logical vector instead of a list. We don't recommend it using it for programming, because the simplification can fail and give you an unexpected type, but it's usually fine for interactive use. `purrr` has a similar function called `map_vec()` that we didn't mention in @sec-iteration.
...
This distinction between `sapply()` and `vapply()` is really important when they're inside a function (because it makes a big difference to the function's robustness to unusual inputs), but it doesn't usually matter in data analysis.
...
Unfortunately `tapply()` returns its results in a named vector which requires some gymnastics if you want to collect multiple summaries and group variables into a data frame (it's certainly possible to not do this and just work with free floating vectors, but in our experience and just delays the work). If you want to see how you might use `tapply()` or other base techniques to perform other grouped summaries, Hadley has collected a few techniques in a [gist](https://gist.github.com/hadley/c430501804349d382ce90754936ab8ec)
...
The most straightforward use of `for` loops is to achieve the same effect as `walk()`: call some function with a side-effect on each element of a list. For example, in @sec-saving-multiple-outputs instead of using `walk()`:
...
```{r}
paths <- dir("book-data/gapminder", pattern = "\\.xlsx$", full.names = TRUE)
...
We recommend avoiding this pattern because it come become very slow when the vector is ver long. This is the source of the persistent canard that `for` loops are slow: they're not, but iteratively growing a vector is.

## Plots

Many R uses who don't otherwise use the tidyverse prefer ggplot2 for plotting due to helpful features like sensible defaults, automatic legends, and a modern look. However, base R plotting functions can still be useful because they're so concise -- it takes very little typing to do a basic exploratory plot.
